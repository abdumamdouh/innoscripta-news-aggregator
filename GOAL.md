...
| 3   | Four live adapters + two stubs                  | needs human review | feat/four-live-adapters-two-stubs     | Not done — acceptance fails: Fixtures: only 2 of 4 providers (guardian.json, bbc-rss.json) are real captured live responses. newsapi.json and nyt.json are hand-authored/invented (dates match the 'today' of the session, content is plausible-fiction, README.md in `__fixtures__` admits this openly: "newsapi.json — Hand-authored... no NEWSAPI_KEY was available" and "nyt.json — Hand-authored... no NYT_KEY available"). This directly fails the acceptance line "every adapter's mapping test passes against a real captured fixture" for 2 of the 4 live adapters. It is documented (not hidden), but the underlying deliverable (a captured NYT/NewsAPI response) simply doesn't exist yet, so this is a real gap, not just a stylistic deviation — the mapping code for those two providers has only ever been tested against data the same author invented to match their own code's expectations, which is weaker evidence of correctness than the brief asked for. |
