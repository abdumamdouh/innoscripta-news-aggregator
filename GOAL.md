...
| 9   | Bookmarks + reading lists (CRUD)                | done   | feat/bookmarks-reading-lists-crud             |                                                                                                                                                                                                                                                                        |
| 10  | Saved search presets (CRUD)                     | done   | feat/saved-search-presets-crud                |                                                                                                                                                                                                                                                                        |
...
| 13  | Docker + CI                                     | done   | feat/docker-ci                                | Verified by hand: image builds, container healthy, GET / and /bookmarks 200 (SPA fallback), nginx proxies /api/bbc 200 and /api/guardian 401 (no key). No keys in the served bundle, none in image history.                                                            |
| 14  | README (single file, submission-ready)          | done   | feat/readme-single-file-submission-ready      |                                                                                                                                                                                                                                                                        |
...
- [x] (minor) Shrunk touch target on saved-page card actions: The 'add to list' and 'remove' AppIconButtons are given className="size-9" (36px), overriding AppIconButton's default size-11 (44px) that every other icon button in the app (Pagination, ArticleDetailsPage's bookmark toggle) uses. No comment explains the exception; on a touch device it's a smaller target right where item 12's responsive pass will be scrutinizing mobile card layouts. — found in 9
      Status: done. Branch: `feat/shrunk-touch-target-on-saved-page-card-a`.
...
- [x] (minor) Responsive pass only lands on the Articles list/details path; other screens rely entirely on transitive fixes: Per GOAL.md item 12, this diff is 'in progress'. The touched files are limited to shared primitives (AppButton, AppCheckbox, AppModal), Header, and the Articles list/details/filter-chips code — Preferences, Feed, Bookmarks, and SavedSearches pages themselves are untouched. They currently pass the new e2e/responsive.spec.ts checks only because they already used flex-col/flex-wrap layouts and shared AppButton/AppIconButton/AppCheckbox primitives that inherited the 44px fix automatically; there's no page-specific responsive work for them in this diff. Worth confirming in a follow-up pass that this transitive coverage is intentional and not an oversight, since the item spec calls out preferences/feed/bookmarks/presets by name as in-scope. — found in 12
      Status: done. Branch: `feat/responsive-pass-only-lands-on-the-articl`.
- [x] (minor) Responsive e2e overflow check isn't exercised against long/unbreakable content: The new overflow() check in e2e/responsive.spec.ts is solid, but the mocked articles it runs against (providerMocks.ts) all use short, similar-length titles/authors. A long unbroken token (long author name, long single-word title) could still overflow the viewport even though line-clamp/wrap classes are in place elsewhere; the current fixtures wouldn't catch that. Consider adding one long-string fixture case to stress this. — found in 12
      Status: done. Branch: `feat/responsive-e2e-overflow-check-isn-t-exer`.
...
- [x] (minor) AppModal drawer variant has no sticky footer for tall content: The drawer variant (`h-dvh` with a single `overflow-auto` region covering title, body and footer) has no sticky/pinned footer. On a short viewport with a long filter form, the drawer's action button (e.g. 'Show results') can scroll out of reach behind the field list before it's clicked. The new e2e/responsive.spec.ts only asserts horizontal overflow and tap-target size, not that the footer stays reachable, so this gap is unverified either way. Worth a follow-up to pin the footer (flex column with a shrink-0 footer) and add a check that the primary action stays visible/reachable in the drawer at 375px. — found in carry-forward-responsive-pass-transitive-coverage
      Status: done. Branch: `feat/appmodal-drawer-variant-has-no-sticky-fo`.
...
- [ ] (major) GOAL.md row 12 has corrupted/duplicated content appended inline: Row 12 ("Responsive pass") in the Backlog table contains a second, garbled copy of what looks like an older version of the same row pasted into the Notes cell: "... | 12 | Responsive pass | in progress | feat/responsive-pass | | `), not `done` — the author's own bookkeeping contradicts ...". This is pre-existing (not touched by the item-14 diff) but corrupts the markdown table and mixes two conflicting statuses (done vs in progress) for the same item. Worth a follow-up cleanup pass on GOAL.md unrelated to item 14. — found in 14
...
iteration 15 — [9] Bookmarks + reading lists (CRUD) — done — static:pass review:pass e2e:pass acceptance:pass
...
iteration 8 — [12] Responsive pass — needs human review — static:pass review:pass e2e:pass acceptance:fail — requeued for another pass
iteration 9 — [carry-forward-responsive-pass-transitive-coverage] Responsive pass only lands on the Articles list/details path; other screens rely entirely on transitive fixes — done — static:pass review:pass e2e:pass acceptance:pass
...
iteration 4 — [carry-forward-appmodal-drawer-sticky-footer] AppModal drawer variant has no sticky footer for tall content — done — static:pass review:pass e2e:pass acceptance:pass
iteration 6 — [14] README (single file, submission-ready) — done — static:pass review:pass e2e:pass acceptance:pass
