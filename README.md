...
| Filter by date, category, source | Filter panel + dismissible chips; state lives in the URL                                                                                                                                                            | `src/features/Articles/components/ArticlesFilters.tsx`, `src/features/Articles/utils/articlesState.ts` · `e2e/articles.spec.ts` "drops every article published outside the chosen date range", "leaves only articles of the chosen category in the grid", "makes a deselected source disappear from the grid entirely" |
...
client-side. BBC RSS is the extreme case: no query, no date range, no pagination, so all three are
applied after the fetch.

Two filters are re-checked even for providers that claim them, because the provider and the card
answer different questions. **Keyword**: the Guardian and NYT search the full body, so a story whose
only mention is three paragraphs down arrives looking unrelated — on a card that reads as a broken
filter. **Date**: providers bound by UTC days, a reader means their own, so east of Greenwich a
range ending 2 August returned stories the card captioned 3 August. Providers are asked a day wide
and the local-day bound decides. Both are pinned in `src/core/sources/tests/aggregator.test.ts` and
asserted against rendered cards in `e2e/articles.spec.ts`.
...
| ------------------------------------------- | ----- | ----- | ------------------ |
| Unit / component (Vitest + Testing Library) | 35    | 416   | `npm test`         |
| End-to-end (Playwright, Chromium)           | 16    | 137   | `npm run test:e2e` |
