...
test.describe('primary navigation', () => {
  test('shows Articles, My feed and Saved, and none of the removed nav strings', async ({
    page,
  }) => {
    await page.goto('/')

    const nav = page.getByRole('navigation', { name: 'Primary' })
    await expect(nav.getByRole('link')).toHaveText(['Articles', 'My feed', 'Saved'])
  ...
  test('keeps the same three items in Arabic', async ({ page }) => {
    await page.goto('/')
    await page.getByRole('combobox', { name: 'Language' }).click()
    await page.getByRole('option', { name: 'العربية' }).click()

    const nav = page.getByRole('navigation', { name: 'التنقل الرئيسي' })
    await expect(nav.getByRole('link')).toHaveText(['المقالات', 'خلاصتي', 'المحفوظات'])
